...
def read_reads(read_fn):
    ...
    for line in f:
        ...
        readReverse = line.split(',')[1][::-1]
        # Clearly, there is room for improvement here.
        all_reads.append(read)
        all_reads.append(readReverse)
        print readReverse
    return all_reads
...
def simple_de_bruijn(sequence_reads, k):
    ...
    return de_bruijn_graph, in_degrees


def de_bruijn_reassemble(de_bruijn_graph, in_degrees):
    """
    ...
    while True:
        ...
        for start in good_starts:
            #print str(len(de_bruijn_graph[start])) + ", " + str(in_degrees[start])
            if len(de_bruijn_graph[start]) == 1 and in_degrees[start] == 0:
                print "found better start"
                current_point = start
                break
...
if __name__ == "__main__":
    ...
    db_graph, in_degrees = simple_de_bruijn(reads, 31)
    for k in db_graph.keys()[:40]:
        print k, db_graph[k]

    output = de_bruijn_reassemble(db_graph, in_degrees)
